...
def initialize_system(request: InitSystemRequest, db: Session = Depends(deps.get_db)):
    ...
    initialize_encryption_service(result["encrypted_dek"], request.master_password, result["salt"])

    return {"message": "System initialized successfully", "status": "success"}


@router.post("/unlock", response_model=InitSystemResponse)
def unlock_system(request: InitSystemRequest, db: Session = Depends(deps.get_db)):
    ...
    try:
        initialize_encryption_service(dek_config.value, request.master_password, salt_config.value)
    except Exception:
