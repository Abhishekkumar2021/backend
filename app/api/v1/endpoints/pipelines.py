...
def list_pipelines(
    ...
    with get_db_session() as db:
        pipelines = PipelineService.get_pipelines(db, skip=skip, limit=limit, status=status)

    logger.info("pipelines_listed", count=len(pipelines))
    return pipelines
...
def create_pipeline(
    ...
    """Create a new data pipeline (initial version created)."""
    logger.info("pipeline_creation_requested", name=pipeline_in.name)

    with get_db_session() as db:
        existing = PipelineService.get_pipeline_by_name(db, name=pipeline_in.name)
        if existing:
            logger.warning("pipeline_name_conflict", name=pipeline_in.name)
            raise HTTPException(
    ...
    logger.info("pipeline_created", pipeline_id=pipeline.id, name=pipeline.name)
    return pipeline
...
def get_pipeline(pipeline_id: int) -> Pipeline:
    ...
    logger.debug("pipeline_retrieval_requested", pipeline_id=pipeline_id)
    ...
    logger.debug("pipeline_retrieved", pipeline_id=pipeline_id)
    return pipeline
...
def update_pipeline(
    ...
    """Update an existing pipeline. Creates a new version if configuration changes."""
    ...
    with get_db_session() as db:
        ...
        updated_pipeline = PipelineService.update_pipeline(db, pipeline_id=pipeline_id, pipeline_in=pipeline_in)
...
def delete_pipeline(pipeline_id: int) -> Pipeline:
...
def get_pipeline_job_history(
    ...
    logger.debug("pipeline_history_requested", pipeline_id=pipeline_id, skip=skip, limit=limit)
    ...
    logger.debug("pipeline_history_retrieved", pipeline_id=pipeline_id, job_count=len(jobs))
    return jobs
...
def list_pipeline_versions(
    ...
    logger.debug("pipeline_versions_list_requested", pipeline_id=pipeline_id, skip=skip, limit=limit)
    ...
    logger.debug("pipeline_versions_listed", pipeline_id=pipeline_id, version_count=len(versions))
    return versions
...
def create_pipeline_version_snapshot(
    ...
    logger.info("pipeline_version_creation_requested", pipeline_id=pipeline_id, description=version_in.description)
    ...
    logger.info("pipeline_version_created", pipeline_id=pipeline_id, version_number=version.version_number)
    return version
...
def get_pipeline_version(pipeline_id: int, version_number: int) -> PipelineVersion:
    """Get a specific pipeline version by number."""
    logger.debug("pipeline_version_retrieval_requested", pipeline_id=pipeline_id, version_number=version_number)
    ...
    if not version:
        logger.warning("pipeline_version_not_found", pipeline_id=pipeline_id, version_number=version_number)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Version {version_number} not found for pipeline",
        )

    logger.debug("pipeline_version_retrieved", pipeline_id=pipeline_id, version_number=version_number)
    return version
...
def restore_pipeline_to_version(pipeline_id: int, version_number: int) -> Pipeline:
    """Restore a pipeline to a previous version."""
    logger.info("pipeline_restore_requested", pipeline_id=pipeline_id, version_number=version_number)
    ...
    logger.info("pipeline_restored", pipeline_id=pipeline_id, restored_to_version=version_number)
...
def activate_pipeline(pipeline_id: int) -> Pipeline:
    """Activate a pipeline for scheduling."""
    logger.info("pipeline_activation_requested", pipeline_id=pipeline_id)
...
def pause_pipeline(pipeline_id: int) -> Pipeline:
    """Pause a pipeline (disable scheduling)."""
    logger.info("pipeline_pause_requested", pipeline_id=pipeline_id)
