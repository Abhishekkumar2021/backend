...
Handles data extraction and loading from/to PostgreSQL databases
"""
import psycopg2
    ...
    def connect(self) -> None:
        ...
        if self._connection is None:
            try:
                ...
                logger.info(f"Connected to PostgreSQL: {self.config['host']}:{self.config.get('port', 5432)}/{self.config['database']}")
...
class PostgreSQLDestination(DestinationConnector):
    ...
    def create_stream(self, stream: str, schema: List[Column]) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to create table {stream}: {str(e)}")
            raise
